const sections = [
  ...
  { title:'AI대학 조직체계', summary:'ADP+X 교육체계 / 학부 및 전공 / AX 융합전공', subs:['ADP+X 교육체계','AI대학 학부 및 전공','AX 융합전공','교육·연구·산업 조직의 연계체계','학부–대학원 연계체계'] },
...
// CSS 변수에 같은 값을 다시 쓰면 브라우저가 해당 하위 트리 스타일을 다시 계산한다.
// 스크롤 애니메이션은 매 프레임 같은 값을 쓰는 경우가 많아 값이 바뀔 때만 통과시킨다.
(function __cssVarGuard(){const sp=CSSStyleDeclaration.prototype.setProperty;
CSSStyleDeclaration.prototype.setProperty=function(name,value,priority){
  if(typeof name==='string'&&name.charCodeAt(0)===45&&name.charCodeAt(1)===45&&!priority){
    if(this.getPropertyValue(name)===String(value))return;
  }
...
function initFacultyDetail(){
  ...
  const reps=[...root.querySelectorAll('[data-faculty-unit]')],heading=root.querySelector('.faculty-group-head h2'),grid=root.querySelector('.faculty-grid'),search=root.querySelector('#faculty-search'),panel=root.querySelector('.faculty-panel'),panelContent=root.querySelector('.faculty-panel-content'),backdrop=root.querySelector('.faculty-panel-backdrop'),unitHome=root.querySelector('.unit-home');
  const personCard=(person,index)=>`<button type="button" class="faculty-person-card" data-faculty-person="${index}"><span class="faculty-mini-portrait" aria-hidden="true">${person.image?`<img src="${facultyEscape(person.image)}" alt="" loading="lazy">`:facultyEscape(facultyInitials(person.name))}</span><span><strong>${facultyEscape(person.name)}</strong><small>${facultyEscape(person.rank)}</small><em>${facultyEscape(person.specialty||'세부전공 정보 없음')}</em><i>${facultyEscape(person.aiCareer||person.career||'AI 관련 주요경력 정보 없음')}</i></span></button>`;
  function showUnit(index,persist=false){previewUnit=index;if(persist)selectedUnit=index;const unit=units[index];reps.forEach((card,i)=>{card.classList.toggle('selected',i===index);card.classList.toggle('dimmed',i!==index);});heading.textContent=`${unit.name} 교수진 · ${unit.faculty.length}명`;const homeUrl=DEPARTMENT_LINKS[unit.name];if(homeUrl){unitHome.href=homeUrl;unitHome.textContent='학과 홈페이지 ↗';unitHome.hidden=false;}else{unitHome.removeAttribute('href');unitHome.hidden=true;}renderGrid();}
...
function renderAdmissionsDetail(){
  ...
  return `<div class="admissions-detail">
    ...
    <section class="admission-section admissions" id="admission-section-1" data-admission-section="0"><div class="admission-stage"><div class="admission-left"><p class="admission-label">01 · ADMISSIONS</p><h1>입학,<br>여기서 시작</h1><p class="admission-subtitle">2027년 3월, 부산대학교 AI대학이 출범합니다.</p></div><div class="admission-right"><div class="admission-copy"><small>2027학년도 수시모집</small><p>2027학년도 부산대학교 수시모집 원서접수는 <strong>2026년 9월 8일부터 11일까지 진행될 예정입니다.</strong></p><p>전형별 모집인원, 지원자격, 전형방법은 부산대학교 입학정보 홈페이지에서 확인합니다.</p></div><div class="calendar-card"><div class="calendar-head"><span>2026 SEPTEMBER</span><b><i data-date-part="year">2026</i><i data-date-part="month">09</i><i data-date-part="range">08 — 11</i></b></div><div class="calendar-days">${[8,9,10,11].map((day,i)=>`<span data-calendar-day="${i}">${String(day).padStart(2,'0')}</span>`).join('')}<i class="calendar-progress"></i></div><p>2027학년도 수시모집 원서접수 예정</p></div><div class="admission-actions"><a class="admission-primary" href="https://go.pusan.ac.kr" target="_blank" rel="noopener">입학정보 확인하기 <span>→</span></a><button type="button" data-admission-go="1">AI대학 모집단위 보기 ↓</button></div></div></div></section>
...
function initCollegeIntroduction(initialChapter=0){
  ...
  const draw=()=>{
    ...
    // 읽기를 먼저, 쓰기를 나중에. 반대 순서면 매 프레임 강제 동기 레이아웃이 발생한다.
    ...
    root.querySelector('.apex-core').style.setProperty('--show',clamp((visionP-.48)/.24));
    const factP=values[2];
    root.querySelectorAll('.fact').forEach((fact,i)=>fact.style.setProperty('--show',clamp((factP-(.08+i*.12))/.22)));
...
function renderOrganizationDetail(){
  ...
  const nodes=[
    ...
    ['unit-x','AX융합학부','27명','ADP 공통역량을 산업·사회 현장에 적용','도메인별 AI 전환 교육 · 산학 프로젝트'],
...
function initOrganizationContinuum(){
  ...
  function state(id,s){
    ...
    if((i=units.indexOf(id))>=0){const isAx=id==='unit-x',mobile=innerWidth<=720,narrow=innerWidth<=800;const unitX=(narrow?[26,74,26,74]:[12,37,63,88])[i],unitY=narrow?[70,70,88,88][i]:80;return [hidden,{x:unitX,y:unitY,o:1,z:.92},{x:isAx?50:18+i*10,y:isAx?(mobile?37:40):80,o:isAx?1:0,z:isAx?1:.6},{x:27+i*11.5,y:84,o:0,z:.54},hidden][s];}
    ...
    if((i=ecos.indexOf(id))>=0){const narrow=innerWidth<=800;const pos=(narrow?[[16,44],[50,44],[84,44],[50,74]]:[[29,50],[50,50],[71,50],[50,73]])[i];return [hidden,hidden,hidden,{x:pos[0],y:pos[1],o:i<3?1:0,z:i<3?1:.7},hidden][s];}
    if((i=rails.indexOf(id))>=0)return [hidden,hidden,hidden,hidden,{x:17+i*22,y:[76,72,76,71][i],o:1,z:.88}][s];
    return hidden;
  }
  let raf=0;
  function draw(){
    raf=0;const rect=root.getBoundingClientRect(),range=Math.max(1,root.offsetHeight-innerHeight),rawOverall=clamp(-rect.top/range)*4,rawIndex=Math.min(3,Math.floor(rawOverall)),rawT=rawOverall-rawIndex,transitionStart=rawIndex===3?.14:.20,transitionSpan=rawIndex===3?.62:.60,transitionT=clamp((rawT-transitionStart)/transitionSpan),transitionEase=transitionT*transitionT*(3-2*transitionT),overall=rawOverall>=4?4:rawIndex+transitionEase,index=Math.min(4,Math.floor(overall)),t=index===4?0:overall-index,ease=t*t*(3-2*t),axPhase=clamp((rawOverall-1.78)/.72);
  ...
  const onHomeClick=event=>{const n=event.target.closest('.org-node[data-home-url]');if(n){event.preventDefault();window.open(n.dataset.homeUrl,'_blank','noopener');}};
